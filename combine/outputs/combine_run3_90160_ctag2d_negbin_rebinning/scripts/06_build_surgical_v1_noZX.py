...
OUTPUT     = "/eos/user/s/snandaku/Higgscharmnew/higgscharm/combine/outputs/combine_run3_90160_ctag2d_negbin_rebinning/rebinning_tests/surgical_v1_noZX"
...
root_path = os.path.join(OUTPUT, "histograms_surgical_noZX.root")
...
dc_path = os.path.join(OUTPUT, "datacard_surgical_noZX.txt")
...
with open(dc_path, "w") as dc:
    ...
    dc.write("imax 1\njmax %d\nkmax *\n" % (len(procs) - 1))
    dc.write("-" * 80 + "\n")
    dc.write(f"shapes * * histograms_surgical_noZX.root h_$PROCESS h_$PROCESS_$SYSTEMATIC\n")
    dc.write("-" * 80 + "\n")
    dc.write("bin         hczz\n")
